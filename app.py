...
translations = {
    "it": {
        ...
        "player_mapping": "Mappatura Giocatori",
        "injury_risk_title": "Predittore Rischio Infortuni",
        "injury_risk_desc": "Analisi AI del rischio infortuni basata su carico di lavoro, asimmetrie e fatica",
        "risk_level": "Livello Rischio",
        "risk_score": "Punteggio Rischio",
        "acwr": "ACWR (Carico Acuto/Cronico)",
        "acwr_desc": "Rapporto tra carico recente (7 giorni) e carico cronico (28 giorni). Ideale: 0.8-1.3",
        "asymmetry": "Asimmetria Laterale",
        "asymmetry_desc": "Differenza tra movimenti destri/sinistri. Ideale: <15%",
        "fatigue": "Indice Fatica",
        "fatigue_desc": "Calo velocita tra inizio e fine sessione. Ideale: <10%",
        "risk_low": "BASSO - Continua cosi",
        "risk_medium": "MEDIO - Monitora attentamente",
        "risk_high": "ALTO - Riduci carico immediato",
        "recommendations": "Raccomandazioni",
        "physical_profile_title": "Profilo Fisico & AI",
        "ai_training_title": "Piano Allenamento Personalizzato AI",
        "ai_nutrition_title": "Piano Nutrizionale Personalizzato AI",
        "offensive_ai_title": "Raccomandatore Giocate Offensive",
        "defense_ai_title": "Ottimizzatore Matchup Difensivi",
        "movement_ai_title": "Analizzatore Pattern Movimento",
        "shot_quality_title": "Simulatore Qualita Tiro",
        "imu_jumps_title": "Rilevamento Salti IMU",
        "team_performance_title": "Performance Squadra",
        "export_reports_title": "Esporta Report"
    },
    "en": {
        "logout": "Logout",
        ...
        "injury_risk_desc": "AI-powered injury risk analysis based on workload, asymmetries and fatigue",
        "risk_level": "Risk Level",
        "risk_score": "Risk Score",
        "acwr": "ACWR (Acute/Chronic Workload Ratio)",
        "acwr_desc": "Ratio between recent load (7 days) and chronic load (28 days). Optimal: 0.8-1.3",
        "asymmetry": "Lateral Asymmetry",
        "asymmetry_desc": "Difference between right/left movements. Optimal: <15%",
        "fatigue": "Fatigue Index",
        "fatigue_desc": "Speed drop between start and end of session. Optimal: <10%",
        "risk_low": "LOW - Keep it up",
        "risk_medium": "MEDIUM - Monitor closely",
        "risk_high": "HIGH - Reduce load immediately",
        "recommendations": "Recommendations",
        "physical_profile_title": "Physical Profile & AI",
        "ai_training_title": "AI Personalized Training Plan",
        "ai_nutrition_title": "AI Personalized Nutrition Plan",
        "offensive_ai_title": "Offensive Play Recommender",
        "defense_ai_title": "Defensive Matchup Optimizer",
        "movement_ai_title": "Movement Pattern Analyzer",
        "shot_quality_title": "Shot Quality Simulator",
        "imu_jumps_title": "IMU Jump Detection",
...
with tab_config:
    ...
    if not use_sample:
        ...
        with col_f2: imu_file = st.file_uploader("IMU CSV", type=['csv'], key='config_imu_file')
    ...
    with col_smtp1:
        ...
        smtp_user = st.text_input("SMTP User", "", key='config_smtp_user')
    with col_smtp2:
    ...
    if smtp_user and smtp_password:
        ...
        smtp_config = None
        st.warning("Email disabled - configure SMTP")
...
# =================================================================
with tab_physical:
    st.header(t("physical_profile_title"))
    st.info("Physical Profile & AI features available in full version")

# =================================================================
# TAB AI ELITE FEATURES - INJURY RISK MIGLIORATO
# =================================================================
with tab_ai:
    st.header(t("tab_ai"))
    ...
    ai_tab1, ai_tab2, ai_tab3, ai_tab4, ai_tab5, ai_tab6 = st.tabs([
        t("injury_risk_title"),
        t("offensive_ai_title"),
        t("defense_ai_title"),
        t("movement_ai_title"),
        t("shot_quality_title"),
        t("imu_jumps_title")
    ])

    with ai_tab1:
        st.subheader(t("injury_risk_title"))
        st.info(t("injury_risk_desc"))
            ...
            with cols[idx % 3]:
                with st.container(border=True):
                    ...
                    <div class='{card_class}'>
                        <h2 style='margin:0;'>{risk_emoji}</h2>
                        <h1 style='margin:10px 0; font-size:48px;'>{risk}/100</h1>
                        <p style='margin:0; font-size:18px;'>{risk_text}</p>
                    </div>
                    ...
                    # ACWR Metric
                    st.markdown(f"**{t('acwr')}**")
                    st.caption(t("acwr_desc"))
                    acwr_status = "Normal" if 0.8 <= acwr <= 1.3 else "Alert"
                    acwr_color = "green" if acwr_status == "Normal" else "red"
                    st.metric("ACWR", f"{acwr:.2f}", delta=acwr_status, delta_color="normal" if acwr_status == "Normal" else "inverse")
                    st.progress(min(acwr / 2, 1.0))

                    st.divider()

                    # Asymmetry Metric
                    st.markdown(f"**{t('asymmetry')}**")
                    st.caption(t("asymmetry_desc"))
                    asym_pct = asym * 100
                    asym_status = "OK" if asym_pct < 15 else "Warning"
                    st.metric("Asymmetry", f"{asym_pct:.1f}%", delta=asym_status, delta_color="normal" if asym_status == "OK" else "inverse")
                    st.progress(min(asym, 1.0))

                    st.divider()

                    # Fatigue Metric
                    st.markdown(f"**{t('fatigue')}**")
                    st.caption(t("fatigue_desc"))
                    fat_status = "OK" if fat < 10 else "High"
                    st.metric("Fatigue", f"{fat:.1f}%", delta=fat_status, delta_color="normal" if fat_status == "OK" else "inverse")
                    st.progress(min(fat / 30, 1.0))

                    # Recommendations
                    if risk > 60:
                        st.error("IMMEDIATE ACTION REQUIRED: Reduce training load by 30-50%")
                    elif risk > 30:
                        st.warning("MONITOR: Consider reducing intensity or volume")
                    else:
                        st.success("GOOD STATUS: Continue current program")

    with ai_tab2:
        st.subheader(t("offensive_ai_title"))
        col1, col2, col3 = st.columns(3)
        with col1: curr_q = st.selectbox("Quarter", [1,2,3,4], index=3, key='ai_off_q')
        with col2: score_diff = st.number_input("Score Diff", -20, 20, -3, key='ai_off_score')
        with col3: curr_spacing = st.number_input("Spacing (m)", 50, 120, 78, key='ai_off_spacing')
    ...
    with ai_tab3:
        st.subheader(t("defense_ai_title"))
    ...
    with ai_tab4:
        st.subheader(t("movement_ai_title"))
        pattern_player = st.selectbox("Select Player", all_players, key='ai_movement_player')
    ...
    with ai_tab5:
        st.subheader(t("shot_quality_title"))
        col1, col2, col3 = st.columns(3)
        with col1: sim_x = st.slider("X Position", 0.0, 28.0, 6.75, 0.5, key='ai_shot_x')
        with col2: sim_y = st.slider("Y Position", 0.0, 15.0, 7.5, 0.5, key='ai_shot_y')
        with col3: sim_spacing = st.slider("Spacing", 50, 120, 75, key='ai_shot_spacing')
        ...
        <div class='ai-report-light' style='border-color:{color};'>
            ...
            <p>qSQ vs League Avg (46%): {(qsq-0.46)*100:+.1f}%</p>
        </div>
        """, unsafe_allow_html=True)

    with ai_tab6:
        st.subheader(t("imu_jumps_title"))
        if imu is None:
            st.info("No IMU data loaded")
        else:
            jumps = imu.get('jump_detected', pd.Series([0])).sum() if 'jump_detected' in imu.columns else 0
...
# =================================================================
with tab_analytics:
    st.header(t("team_performance_title"))
    ...
    st.divider()
    st.subheader(t("export_reports_title"))

    col_exp1, col_exp2 = st.columns(2)
    with col_exp1:
        st.markdown("**Team Report**")
        team_pdf = generate_team_pdf(team_name, kpi, brand_color, session_type if session_type == "Match" or session_type == "Partita" else "Training")
        if team_pdf:
            st.download_button("Download Team PDF", data=team_pdf, file_name=f"{team_name}_Team_Report.pdf", mime="application/pdf", key='analytics_team_pdf')

    with col_exp2:
        st.markdown("**Player Report**")
        player_pdf_sel = st.selectbox("Select Player", all_players, key='analytics_player_pdf_sel')
        player_pdf = generate_player_pdf(player_pdf_sel, uwb[uwb['player_id']==player_pdf_sel], team_name, brand_color, session_type if session_type == "Match" or session_type == "Partita" else "Training")
        if player_pdf:
            pname = st.session_state.player_names.get(player_pdf_sel, player_pdf_sel)
            st.download_button(f"Download {pname} PDF", data=player_pdf, file_name=f"{pname}_Report.pdf", mime="application/pdf", key='analytics_player_pdf')
    ...
    st.download_button("Export KPI CSV", data=csv, file_name=f"{team_name}_KPI.csv", mime="text/csv", key='analytics_csv')
