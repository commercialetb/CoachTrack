# =================================================================
# COACHTRACK ELITE AI v3.3 - CV AI ADVANCED MODULE
# FIXED: Compatible with MediaPipe 0.10.30+ (NO solutions module)
# =================================================================
...
class ActionRecognizer:
    ...
    def process_video_actions(self, pose_data: List[Dict]) -> List[Dict]:
        """Processa video e restituisce timeline azioni"""
        actions_timeline = []
        ...
        for i in range(0, len(pose_data), self.window_size):
            ...
            action_result['timestamp'] = window[0].get('frame', 0) / 30.0
...
# =================================================================
# 3️⃣ POSE ESTIMATION - FIXED FOR 0.10.30+
# =================================================================

class PoseAnalyzer:
    """Analizza pose con MediaPipe 0.10.30+"""
    ...
    def __init__(self):
        ...
        if MEDIAPIPE_AVAILABLE and mp_pose is not None:
            try:
                # Use Pose class from mp_pose module
                ...
                self.available = True
                print("✅ MediaPipe Pose detector inizializzato!")

            except Exception as e:
                self.available = False
                print(f"⚠️ Errore init Pose: {e}")
        else:
    ...
    def process_video_complete(self, video_path: str, output_json: str = "cv_ai_analysis.json") -> Dict:
        ...
        while True:
            ...
            if frame_idx % 100 == 0:
                ...
                print(f"   Progress: {progress:.1f}% - Poses: {len(all_poses)}")
...
__version__ = "3.3.0"
__status__ = "FIXED for MediaPipe 0.10.30+ (python.solutions)"
__all__ = [
    'ActionRecognizer',
    'ShotTracker',
    'PoseAnalyzer',
